...
void Gnss::nmeaCb(GpsUtcTime timestamp, const char* nmea, int length) {
    ...
    // CAF loc_eng_nmea_put_checksum on this MPSS returns a length that omits
    // the leading '$'. hidl_string::setToExternal CHECKs data[length] == NUL
    // and SIGABRTs the GNSS HAL. CAF already NUL-terminates the buffer.
    if (nmea == nullptr) {
        return;
    }
    (void)length;
    android::hardware::hidl_string nmeaString = nmea;
